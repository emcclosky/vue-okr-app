    ...
    <div class="password-reset-form__container">
      <div class="password-reset-form__inner-container">
        ...
        <p class="password-reset-form__description" v-if="submitted === true">
          A reset link has been sent to {{ email }} with instructions on how to
          reset your password.
        </p>
        ...
        <div
          class="password-reset-form__form-container"
...
export default {
  ...
  methods: {
    ...
    async handlePasswordReset() {
      try {
        const response = await this.$store.dispatch("sendPasswordReset", {
          email: this.email,
        });
        if (response.status === 204) this.submitted = true;
      } catch (error) {
        console.error(error);
      }
    },
