...
async function googleStrategy(accessToken, refreshToken, profile, done) {
	try {
		...
		console.error('error from googleStrategy: ', err);
		return done(err);
	}
}
...
const postPasswordReset = async(req, res, next) => {
  ...
  crypto.randomBytes(32, (err, buffer) => {
    if(err) {
      console.error('error in crypto.randomBytes', err);
  ...
  try {
    ...
    RESET_CACHE[email] = { resetToken: token, resetExpiration: Date.now() + 600000 };
...
const getNewPassword = (req, res, next) => {
  ...
  if(!(user in RESET_CACHE)) return res.sendStatus(404);
    ...
    if(currentUser.token === token && currentUser.resetExpiration >= Date.now())
      return res.sendStatus(200);
    else
